...
const getStatistics1 = async () => {
  const res = await userApi.getStatistics1()
  statistics1.value = res.panels
}
const getStatistics2 = async () => {
  const res = await userApi.getStatistics2()
...
const init = async () => {
  ...
  await userApi.getStatistics3(current.value).then((res) => {
    // 使用刚指定的配置项和数据显示图表。
